...
already in the sre folder from the assignment zip file. This means that you
...
- If the player has a Invincibility component, it takes no damage from
...
- When the player collides with an enemy while not invincible:
  ...
  - player is given a Invincibility component which lasts for 30 frames
...
- BoundingBox has two new booleans to reflect these changes
...
- Tiles will be given a BoundingBox equal to the size of the animation
...
ok 1. Implement WASD movement for the player such that it moves u/1/d/r when
   those keys are pressed. This will allow you to test rectangular collisions
   as soon as possible
ok 2. Implement GameState_Play::loadLevel(), which will allow you to load the
   assets for the level given to you to test the rest of the assignment
ok 3. Implement spawnPlayer) so that it correctly uses the configuration
   specified by the level file so you can start testing collisions
ok 4. Implement the camera changes described above
ok 5. Implement spawnSword() so that it spawns a sword in the correct position
   with the correct lifespan
ok 6. Implement player attacking / sword collision with NPCs / health / damage
ok 7. Implement player animations to correctly animate based on player state
ok 8. Implement NPC Patrol behavior
9. Implement NPC Follow Behavior without vision
10.Implement Physics::lineIntersect() which will allow you to test whether or
   not two line segments AB and CD intersect, and where they intersect
11.Implement Physics::EntityIntersect() which will allow you to test whether
   line segment AB intersects with any lines of the bounding box of Entity e
12.Implement NPC Follow Behavior using 10 to check line of sight
ok 13.Implement invincibility frames
ok 14.Implement heart pickups
ok 15.Implement black tile teleporting
16.Create custom level